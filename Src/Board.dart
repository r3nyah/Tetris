...
import '../Src/Touch.dart';
import '../Src/Rotation.dart';
import '../Src/Piece.dart';
...
class Board extends ChangeNotifier {
  ...
  bool isBlocked(Vector v) {
    return _blocked.contains(v);
  }

  bool isCurrentPieceTile(Vector v) {
    return currentPiece.tiles.contains(v - _cursor);
  }

  bool isFree({Vector offset = Vector.zero}) {
    return currentPiece.tiles.where((v) {
      return _blocked.contains(v + _cursor + offset);
    }).isEmpty;
  }

  bool inBounds({Vector offset = Vector.zero}) {
    return currentPiece.tiles.where((v) {
          return v + _cursor + offset >= const Vector(x, y);
        }).isEmpty &&
        currentPiece.tiles.where((v) {
          return v + _cursor + offset < Vector.zero;
        }).isEmpty;
  }
  ...
  bool canMove(Vector offset) {
    return inBounds(offset: offset) && isFree(offset: offset);
  }
  ...
  bool isBlockOut() {
    return _blocked.where((e) => e.y == y - 1).isNotEmpty;
  }
